...
object ChannelRepository {
    ...
    suspend fun getAllChannels(context: Context): List<Channel> {
        cache?.let { return it }
        ...
        // Fill in logos for channels that only ship a tvg-id (no tvg-logo)
        // by looking them up in the public iptv-org logo database.
        val resolved = all.map { channel ->
            if (channel.logoUrl.isNullOrBlank() && !channel.tvgId.isNullOrBlank()) {
                channel.copy(logoUrl = TvgLogoResolver.resolve(channel.tvgId))
            } else {
                channel
            }
        }
    ...
    suspend fun getGroups(context: Context): List<String> =
        getAllChannels(context).map { it.group }.distinct()

    suspend fun getByGroup(context: Context, group: String): List<Channel> =
        getAllChannels(context).filter { it.group == group }

    suspend fun search(context: Context, query: String): List<Channel> {
